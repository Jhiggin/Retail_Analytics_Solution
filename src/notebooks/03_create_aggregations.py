...
df_daily_sales = (
    df_sales.join(df_products, "product_id", "left")
    .join(df_customers, "customer_id", "left")
    .groupBy("transaction_date", "transaction_year", "transaction_month", "store_id", "category")
    .agg(
...
df_customer_ltv = (
    df_sales.join(df_customers, "customer_id", "left")
    ...
    .withColumn(
        "customer_tenure_days", datediff(col("last_purchase_date"), col("first_purchase_date"))
    )
    .withColumn("processed_timestamp", current_timestamp())
)
...
df_product_performance = (
    df_sales.join(df_products, "product_id", "left")
    .groupBy("product_id", "product_name", "category", "subcategory", "brand")
    .agg(
        count("transaction_id").alias("total_transactions"),
...
df_monthly_trends = (
    df_sales.groupBy("transaction_year", "transaction_month", "transaction_quarter")
    .agg(
        count("transaction_id").alias("total_transactions"),
        countDistinct("customer_id").alias("unique_customers"),
        sum("amount").alias("total_revenue"),
    ...
    .withColumn(
        ...
        (
            col("total_revenue")
            - lag("total_revenue").over(Window.orderBy("transaction_year", "transaction_month"))
        )
        / lag("total_revenue").over(Window.orderBy("transaction_year", "transaction_month"))
        * 100,
    )
